...
function handle_get(req,res) {
  ...
  console.log("substring: " + req.url.substring(1));
  readone(res, req.url.substring(1));
}

function handle_post(req,res,body) {
    console.log("post body: ", JSON.parse(body[0].toString()))
    let nameOfPet = JSON.parse(body[0].toString()).name;
    const petItems = {
        species: JSON.parse(body[0].toString()).species,
        breed: JSON.parse(body[0].toString()).breed,
        age: JSON.parse(body[0].toString()).age
    }
  createone(res, nameOfPet, petItems);
}
...
function handle_delete(req,res) {
    console.log("delete body: ", req.url.substring(1))
